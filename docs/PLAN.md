...
| **2.5 — CLI prototype for the brain/tool loop (parallel track)** | Build a lightweight command-line front-end to the *same* FastAPI backend/agent loop you're already running — no voice, no Flutter, just a terminal chat loop that talks to Ollama (offline) and the Claude API (online) and can call 1–2 dummy tools (e.g., read a file, get system stats). Persist conversation history to a local JSON/SQLite file between runs. | 3–5 days | You can `python jarvis_cli.py`, chat with it in a terminal, and watch it call a tool and return a result — proves the brain + tool-calling loop works *before* you wire voice or the Flutter UI on top of it |
...
**Why Phase 2.5 is worth the detour:** you're currently in Phase 2 wiring up wake-word/VAD/tray behavior, which is voice-pipeline work and doesn't touch the brain or tool-calling loop at all. A small CLI version of the assistant is the cheapest possible way to get the agent loop (LLM → decide → call tool → respond) working end-to-end in isolation, in a plain terminal, with none of the voice or UI complexity in the way. When you get to Phase 3–4, you're extending a loop you already know works instead of building it for the first time under the added pressure of also wiring it into Flutter and the mic pipeline. It's optional, small, and throwaway-safe — you can literally delete `jarvis_cli.py` once Phase 4 subsumes it, or keep it around as a fast debug console for the backend.
...
| **CLI prototype (Phase 2.5)** | `typer`/`click` (CLI), `rich` (terminal formatting), same `anthropic` SDK + Ollama base-URL swap as the real backend | — |
...
2. **Agent/tool-use design patterns** — writing good JSON-schema tool definitions and an agent loop that reliably picks the right tool (or correctly does nothing) is a skill in itself, distinct from general LLM prompting. Expect Phase 3–4 to involve real iteration here. (The Phase 2.5 CLI prototype is a low-stakes place to start practicing this.)
